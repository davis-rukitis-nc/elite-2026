...
function App() {
  ...
  return <>
    ...
    <main className="app">
      ...
      <div className="desktop-view"><div className="table-shell"><table className="elite-table"><thead><tr><th>BIB</th><th>Athlete</th><th>Personal best</th><th>Race</th></tr></thead><tbody>{filtered.map((athlete) => { const flag = flagEmoji(athlete.countryCode, athlete.country); return <tr key={athlete.id} onClick={() => setSelected(athlete)}><td className="bib-cell">{athlete.bib || "—"}</td><td><div className="athlete-cell"><div className="name-cell">{athlete.name}</div><div className="table-meta"><span className="table-country">{flag ? `${flag} ` : ""}{athlete.country || "—"}</span><span className="table-tags"><span className="table-chip">{athlete.gender}</span><span className="table-chip">{athlete.year || "—"}</span></span></div></div></td><td><div className="pb-cell"><strong className="pb-time">{athlete.pb || "—"}</strong><span className="pb-event">{athlete.event || "—"}</span></div></td><td><div className="race-cell"><span className={`distance-pill ${athlete.distanceColorClass}`}>{athlete.distance}</span><span className="profile-cue">Profile →</span></div></td></tr>; })}</tbody></table></div></div>
