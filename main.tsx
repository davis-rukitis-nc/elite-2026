...
function App() {
  ...
  return <>
    ...
    <main className="app">
      ...
      <div className="desktop-view"><div className="table-shell"><table className="elite-table"><thead><tr className="top-head"><th>BIB</th><th>Athlete</th><th>Country</th><th>Year</th><th className="pb-head" colSpan={2}>Personal Best</th><th>Distance</th><th>Gender</th></tr><tr className="sub-head"><th>No.</th><th>Name</th><th>Flag / nation</th><th>Born</th><th>Time</th><th>Event</th><th>Race</th><th>M / W</th></tr></thead><tbody>{filtered.map((athlete) => { const flag = flagEmoji(athlete.countryCode, athlete.country); return <tr key={athlete.id} onClick={() => setSelected(athlete)}><td>{athlete.bib || "—"}</td><td className="name-cell">{athlete.name}</td><td>{flag ? `${flag} ` : ""}{athlete.country || "—"}</td><td>{athlete.year || "—"}</td><td className="pb-time">{athlete.pb || "—"}</td><td>{athlete.event || "—"}</td><td><span className={`distance-pill ${athlete.distanceColorClass}`}>{athlete.distance}</span></td><td>{athlete.gender}</td></tr>; })}</tbody></table></div></div>
