...
const flagEmoji = (code: string) => {
  ...
  const isoMap: Record<string, string> = {
    ETH: "ET", KEN: "KE", TAN: "TZ", UGA: "UG", GER: "DE", ERI: "ER", LAT: "LV",
    LTU: "LT", EST: "EE", FIN: "FI", SWE: "SE", NOR: "NO", DEN: "DK", POL: "PL",
    UKR: "UA", MAR: "MA", RWA: "RW", BDI: "BI", FRA: "FR", ESP: "ES", ITA: "IT",
    POR: "PT", GBR: "GB", USA: "US"
  };
...
const normalizeDistance = (distance: string) => {
  ...
  if (lower === "marathon" || value === "42KM" || value === "42K" || value === "42 km") return "Marathon";
  if (lower === "half marathon" || value === "21KM" || value === "21K" || value === "21 km") return "Half marathon";
  if (value === "10KM" || value === "10K" || value === "10 km") return "10 km";
  if (value === "5.7KM" || value === "5.7K" || value === "5.7 km") return "5.7 km";
  if (lower === "mile") return "Mile";
  return value || "—";
};
    ...
    <div className="select-wrap" ref={ref}>
      ...
      {open && (
        <div className="select-menu" role="listbox">
          {options.map((option) => (
            <button className={option === value ? "select-item active" : "select-item"} type="button" key={option} onClick={() => { onChange(option); setOpen(false); }}>
              {option}
            </button>
          ))}
        </div>
...
function AthleteRow({ athlete, onOpen }: { athlete: Athlete; onOpen: (athlete: Athlete) => void }) {
  return (
    <button className="athlete-row" type="button" onClick={() => onOpen(athlete)}>
      <div className="bib">{athlete.bib || "—"}</div>
      <div className="runner">
        <AthleteInitials athlete={athlete} />
        <div className="runner-text">
          <strong>{athlete.name}</strong>
          <span>{flagEmoji(athlete.countryCode)} {athlete.country || athlete.countryCode}</span>
        </div>
      </div>
      <div className="meta distance"><span>{athlete.distance}</span></div>
      <div className="meta gender"><span>{athlete.gender}</span></div>
      <div className="pb">
        <strong>{athlete.pb || "—"}</strong>
        <span>{athlete.pbEvent || "Personal best"}</span>
      </div>
    </button>
  );
}
    ...
    <div className="modal-layer" onMouseDown={onClose}>
      <div className="profile-modal" role="dialog" aria-modal="true" aria-label={`${athlete.name} profile`} onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="profile-head">
          <AthleteInitials athlete={athlete} />
          <div>
            <p className="profile-kicker">{athlete.bib ? `BIB ${athlete.bib}` : "Elite runner"} · {athlete.distance}</p>
            <h2>{athlete.name}</h2>
        ...
        {athlete.tags.length > 0 && <div className="tags">{athlete.tags.map((tag) => <span key={tag}>{tag}</span>)}</div>}
...
function App() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const parsed = Papa.parse<RawRow>(text, { header: true, skipEmptyLines: true, delimiter: ";" });
    ...
    <>
      <header className="standalone-bar">
        <a href="https://rimirigamarathon.com/" target="_blank" rel="noreferrer" aria-label="Open Rimi Riga Marathon">
          <img src="https://rimirigamarathon.com/wp-content/uploads/2024/01/rrm-logo-white.svg" alt="Rimi Riga Marathon" />
        </a>
        <a href="https://rimirigamarathon.com/" target="_blank" rel="noreferrer">Back to rimirigamarathon.com</a>
      </header>
      ...
      <main className="app">
        ...
        <section className="athlete-list" aria-label="Elite runners">
          {filtered.map((athlete) => <AthleteRow key={`${athlete.bib}-${athlete.name}`} athlete={athlete} onOpen={setSelected} />)}
        </section>
